...
<template>
  <div class="c">
    <div class="filter">
      <el-input v-model="options.filterKeys.title" placeholder="검색어 입력" style="width: 40%" clearable>
        <template #prepend>제목</template>
      </el-input>
      <el-input v-model="options.filterKeys.articleEditor" placeholder="검색어 입력" style="width: 40%" clearable>
        <template #prepend>작성자 (표기용)</template>
      </el-input>
        ...
        <table cellpadding="0" cellspacing="0">
          ...
          <thead>
            <tr>
              ...
              <th>작성자(표기용)</th>
              <th @click="sortKey('hits')" class="on">
                조회수
                <sort :arrow="options.sortKeys.hits"></sort>
              </th>
              <th @click="sortKey('openTime')" class="on">
                오픈일시
                <sort :arrow="options.sortKeys.openTime"></sort>
              </th>
              <th>최종 수정</th>
          ...
          <tbody v-if="boardData.filteredData.length > 0">
            <tr v-for="entry in boardData.filteredData">
              ...
              <td class="left normal">
                <router-link :to="'article/modify/' + entry.articleSeq" custom v-slot="{ href, navigate }">
              ...
              <td class="link" v-html="entry.articleEditor"></td>
              <td>{{ entry.hits }}</td>
              <td class="number">
                {{ entry.openTime ? entry.openTime.toDate("YY/MM/DD HH:mm") : "" }}
              </td>
              <td class="normal">
                {{ entry.modifyIdLabel }}
              ...
              <td>
                <revision-history serviceName="report" menuPath="article" :menuKey="entry.noticeSeq"></revision-history>
          ...
          <tfoot>
            <td colspan="100">
              <div class="pagination">
                ...
                <router-link :to="{ path: 'article/add' }"><el-button type="primary" class="add">등록</el-button></router-link>
              </div>
            </td>
          </tfoot>
        </table>
...
emit("title", `${title} 관리`, `리포트 아티클 콘텐츠를 등록 관리 합니다.`);

let labels = reactive({});
let mergeKeys = reactive({ modifyId: "registerId", modifyTime: "registerTime" });

function setLabels() {
  labels.useYn = metaData.value.articleUseYn;
...
let { data } = fetch("/api/report/article", { size: 10000, sort: "articleSeq,desc" });
...
function deleteRow(row) {
  ElMessageBox.confirm(`"${row.title}"을 삭제하시겠습니까?`, `${title} 삭제`, {
    ...
    let { data } = fetch("/api/report/article/" + row.articleSeq, {}, {}, "delete");

    watch(data, async () => {
      if (data) {
        await fetchAsync("/api/report/articlereport/" + row.articleSeq, {}, {}, "delete");
        await fetchAsync("/api/report/articletag/" + row.articleSeq, {}, {}, "delete");

        del({ key: "articleSeq", value: row.articleSeq });
        ElMessage({
          type: "info",
